...
2. **配置单一来源**：`rag-server/config/settings.yaml` 控制 Provider、top_k、默认 `collection_name`、重排开关等；Client 侧只负责拉起进程，不在 MCP 参数里重复一份密钥（Key 配在 rag-server 内）。**注意**：部分 MCP Tool 的默认 `collection`（如 `query_knowledge_hub` → `travel_plan`）与 yaml 的 `collection_name` 可能**不是同一配置项**，多库场景应显式传参（见 §3.7）。  
...
| 能力 | 说明 |
|------|------|
| 发现知识库 | MCP Tool `list_collections`：**合并** Chroma 实库列表 + 本项目 `config/collections.yaml` 注册说明（≠ 直接透传 Chroma `list_collections()` API） |
| 混合检索 | 通用范式 Dense + BM25 + RRF（+ 可选 Rerank）；由本项目 `HybridSearch` 编排，**仅返回 Top-K chunk，不生成回答** |
| 文档摘要 | 按本项目 `doc_id`（文件 SHA256 或哈希片段）取标题、摘要预览、标签、来源路径 |
| 多模态片段 | 命中含图片的 chunk 时，响应可带 MCP 标准 `ImageContent` 块 |

入库、批量评估、改配置等运维动作通过 **CLI 或 Dashboard** 完成，不通过 MCP Tools 暴露。

### 3.2 设计原则

- **传输**：**MCP stdio**（行业标准子进程传输），无 HTTP 端口。  
- **SDK**：官方 Python `mcp` 包 + 本项目 `ProtocolHandler` 注册 `tools/list`、`tools/call`。  
...
> MCP 协议只规定 `tools/list`、`tools/call` 等调用机制；下表 Tool 名与语义由 **rag-server 自定义**（项目内称「三件套」），换用其他 MCP Server 时名称与参数可能完全不同。

| 工具 | 主要参数 | 作用 |
|------|----------|------|
| `list_collections` | `include_stats`（默认 true） | 返回各 collection 名称；可选 chunk/文档计数；合并 yaml 注册说明 |
