...
分层架构：**Ingestion → Core（检索）→ Observability / MCP 接口**；数据落 **Chroma（Dense）+ 自研 BM25（Sparse）+ SQLite（入库历史/完整性）**；检索为 **Hybrid Search（RRF）+ 可选 Rerank**；对外 **MCP stdio 三件套**；全链路 **Trace JSONL + Streamlit Dashboard**。
...
1. **入库**：文件 → Loader → Splitter/Chunker → Transform → Embedding → Chroma + BM25（按 `collection` 分目录）  
2. **检索**：Query → Dense ∥ Sparse → RRF →（可选）Rerank → Top-K + Citation  
3. **观测**：各 stage 写入 Trace；Dashboard 读取展示；评估结果写入 eval_history  
...
| 项 | 内容 |
|----|------|
| **最终选型** | `SparseEncoder`（**jieba** 分词 + 词频统计）→ `BM25Indexer` 落盘 `data/db/bm25/{collection}/` |
...
| 项 | 内容 |
|----|------|
| **最终选型** | `TraceContext` + `TraceCollector`；入库/检索各 stage 记耗时与快照；追加写入 `logs/traces.jsonl` |
...
| 项 | 内容 |
|----|------|
| **最终选型** | `CustomEvaluator`：对比 `expected_chunk_ids` 计算 **hit_rate**、**MRR** |
...
| 项 | 内容 |
|----|------|
| **最终选型** | `RagasEvaluator` 包装 **faithfulness / answer_relevancy / context_precision**；支持 `ragas_chinese_prompts`、百炼 JSON 模式兼容 |
...
| 工具 | 职责 |
|------|------|
| `list_collections` | 发现可用知识库及统计 |
| `query_knowledge_hub` | HybridSearch + Rerank，返回 citation 结构化结果 |
| `get_document_summary` | 单文档摘要与元信息 |
