...
**Golden Test Set → HybridSearch 检索 →（可选）生成/填写 Answer → Evaluator 打分 → 写入 `eval_history.jsonl` → 面板对比迭代。**

检索质量看 **Custom（hit_rate / MRR）**；回答质量看 **Ragas（faithfulness 等）**；**composite** 一次跑通两类指标。
...
选型背景见 [系统架构与模块选型.md](./系统架构与模块选型.md) §5；踩坑与排障见 [RAG评估调优专题.md](../test-analysis/RAG评估调优专题.md)。
...
| | **Custom Evaluator** | **Ragas Evaluator** |
|--|----------------------|---------------------|
...
**composite**：默认子后端 `custom + ragas`，合并 metrics；某一子后端失败时写入 `warnings`，其余指标仍保留。
...
```
evaluation_panel._execute_evaluation()
  → EvaluatorFactory.create(settings)     # custom | ragas | composite
  → _try_create_hybrid_search(collection) # 与线上一致的 HybridSearch
  → EvalRunner.run(golden_path)
       对每题：
         hybrid_search.search(query)     # + 可选 reranker
         answer = 面板覆盖 | 拼接 chunk  # 占位生成
         evaluator.evaluate(query, chunks, answer, ground_truth)
  → EvalReport.to_dict()
  → _save_to_history()                    # logs/eval_history.jsonl
```
...
| 模块 | 路径 |
|------|------|
| 批跑编排 | `src/observability/evaluation/eval_runner.py` |
| 工厂 | `src/libs/evaluator/evaluator_factory.py` |
| Custom | `src/libs/evaluator/custom_evaluator.py` |
| Ragas | `src/observability/evaluation/ragas_evaluator.py` |
| 中文 Prompt | `src/observability/evaluation/ragas_zh_prompts.py` |
| Composite | `src/observability/evaluation/composite_evaluator.py` |
| 面板 | `src/observability/dashboard/pages/evaluation_panel.py` |

### 3.2 Golden Test Set

默认路径：`tests/fixtures/golden_test_set.json`。
...
| 场景 | 主要沉淀 | 说明 |
|------|----------|------|
| **MCP / 手动 query** | `logs/traces.jsonl` + 「检索记录」 | 含 dense / sparse / fusion / rerank 各 stage |
| **批量 Golden Set 评估** | `logs/eval_history.jsonl` + 评估面板 | `EvalReport` 含每题 `retrieved_chunk_ids`、metrics、warnings |
...
页面：**效果评测**（`evaluation_panel.py`）。操作步骤见 [管理面板指南.md](../管理面板指南.md)。
...
排障顺序见 [RAG评估调优专题.md](../test-analysis/RAG评估调优专题.md) §排障路径。
