    ...
    if "rerank" not in stages_by_name:
        if dense_count > 0 or sparse_count > 0:
            hints.append((
                "info",
                "**重排序已跳过：** 重排序器未启用或未配置。"
                "可在 settings.yaml 中启用 `reranker` 以应用基于 LLM 的重排序。",
            ))
...
def _render_evaluate_button(trace: Dict[str, Any], idx: int) -> None:
    ...
    with col_btn:
        clicked = st.button(
            ...
            help="重新执行此查询并使用 Ragas (LLM-as-Judge) 评分",
