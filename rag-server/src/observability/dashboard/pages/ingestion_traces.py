    ...
    for idx, trace in enumerate(traces):
        ...
        with st.expander(expander_title, expanded=(idx == 0)):
            ...
            if main_stages:
                st.markdown("#### ⏱️ 各阶段耗时")
                chart_data = {
                    _STAGE_LABELS.get(t["stage_name"], t["stage_name"]): t["elapsed_ms"]
                    for t in main_stages
                }
                st.bar_chart(chart_data, horizontal=True)
                st.table([
                    {
                        "阶段": _STAGE_LABELS.get(t["stage_name"], t["stage_name"]),
...
def _render_ingestion_diagnostics(
    ...
    if "load" in stages_by_name and load_d.get("text_length", 0) == 0:
        st.warning("**加载阶段未提取到文本。** 文档可能仅含图片，或格式不受支持。")

    if "split" in stages_by_name and split_d.get("chunk_count", 0) == 0:
        st.warning("**分块阶段产出 0 个分块。** 文档文本可能过短或为空。")
...
def _render_embed_stage(data: Dict[str, Any]) -> None:
    ...
    with dense_tab:
        st.markdown("每个分块 → 通过嵌入模型生成**浮点向量**（如 `text-embedding-ada-002`）")
        dense_rows = []
        for i, chunk in enumerate(chunks):
            char_len = chunk.get("char_len", 0)
            dense_rows.append({
                ...
                "字符数": char_len,
                "估算 Token": max(1, char_len // 3),
                "稠密维度": chunk.get("dense_dim", data.get("dense_dimension", "—")),
            })
        st.table(dense_rows)

    with sparse_tab:
        st.markdown("每个分块 → 生成**词频统计**用于 BM25 索引")
        sparse_rows = []
        for i, chunk in enumerate(chunks):
            sparse_rows.append({
                "#": i + 1,
        ...
        for i, chunk in enumerate(chunks):
            ...
            if top_terms:
                with st.expander(f"🔤 分块 {i + 1} — 高频词项", expanded=False):
