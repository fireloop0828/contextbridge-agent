# rag-server 技术亮点（B 轨，简历用）

> 从 `rag-server/DEV_SPEC` 与源码提炼。原 `project_highlights.md` 迁入。按需选取 3-5 条。
...
- 内置 5 大 Agent Skill 覆盖完整开发生命周期：
  - **auto-dev**：读取 DEV_SPEC 中的任务定义与架构规范，自动实现代码并运行测试，支持最多 3 轮自动修复
  - **run-qa**：基于 QA_TEST_PLAN 自动执行全类型测试（CLI / Dashboard UI / MCP 协议 / Provider 切换），自动诊断并修复失败用例
  - **setup**：交互式一键配置向导，覆盖 Provider 选择 → API Key → 依赖安装 → 配置生成 → Dashboard 启动，失败自动诊断重试
  - **clean-project**：一键清理打包，移除缓存/构建产物/敏感信息，生成可分发代码包
  - **write-resume**：结合项目亮点与用户画像，自动生成定制化简历项目经历
...
- "构建 5 大 Agent Skill 体系（auto-dev / run-qa / setup / clean-project / write-resume），覆盖编码、测试、配置、打包、文档生成全生命周期，开发效率提升数倍"
