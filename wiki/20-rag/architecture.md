...
sources:
  - rag-server/src/core/query_engine/
  - rag-server/src/ingestion/
  - rag-server/src/libs/
related: [rag/requirements, rag/flow, rag/mcp-server]
updated: 2026-09-10
---

# rag-server 系统架构与模块选型

> **TL;DR**：rag-server 按「入库 / 检索 / 可观测 / 评估 / MCP 对外」五条链路分别选型，并以 Factory + YAML 实现全链路可插拔：入库为 Loader→检查→切分→增强→向量化→BM25→Upsert，检索为预处理→Dense+Sparse→RRF→Rerank。
...
更简版流程见 [整体设计和流程.md](./flow.md)。

---

## 2. 入库链路选型

每项含 **目的**、**最终选型**（在本项目中的功能）、**备选缺陷**。
...
| **最终选型**  | 按扩展名工厂路由：`PdfLoader` = **MarkItDown** 抽 PDF 正文转 Markdown + **PyMuPDF（`fitz`）** 抽内嵌图落盘、插入 `[IMAGE: id]` 占位；`TxtLoader`、`MarkItDownLoader`（docx/md 等）→ `get_loader_for_path`；图片目录 `data/images/{collection}/{doc_hash}/`；Dashboard/CLI 同路径入库 |
| **备选缺陷** | **纯 PyMuPDF 作唯一加载器**：难与 docx/md 共用 MarkItDown 管线、正文策略分散；**纯 MarkItDown**：不负责本项目内嵌图落盘与占位；Unstructured：重依赖、部署重；按格式写死脚本：每加格式改多处。**运维注意**：PyMuPDF 为 PDF **图片抽取**的运行时依赖（`pyproject.toml` 当前仅声明 `markitdown[pdf]`）；未安装时 `PdfLoader` **降级为仅文本**（日志 warning，图片数 0），后续 `ImageCaptioner` 无图可 caption |
...
| **备选缺陷** | 每次全量重跑：浪费 API 与耗时；仅比文件名：重命名/拷贝会误判                                                   |
...
| **备选缺陷** | 定长切分：易断句；按页切：块大小不均；Semantic Chunking：需额外模型与延迟                                                                                        |
...
| **最终选型**  | `ChunkRefiner`（去噪）、`MetadataEnricher`（标题/关键词）、`ImageCaptioner`（Vision LLM 将图写入文本）；配置开关，失败可降级。**前提**：PDF 图示需 Loader 阶段 PyMuPDF 已抽图并写入 `[IMAGE: id]`，否则 Caption 阶段无输入 |
| **备选缺陷** | 仅原文入库：PDF 图表不可搜；独立多模态向量库：架构分叉、运维成本高                                                          |
...
| **备选缺陷** | 本地 sentence-transformers 常驻：占内存、模型选型绑定机器；单条同步：入库慢、易触发限流                                  |
...
| **备选缺陷** | Elasticsearch：需独立集群；Chroma 内置稀疏：版本/能力不确定；仅 Dense：专名与缩写召回弱                                                   |
...
| **备选缺陷** | Milvus/Qdrant/pgvector：私有本地场景运维更重；本项目优先零端口嵌入式部署                                                                        |
...
| **备选缺陷** | 仅 Chroma 元数据：文档级运维弱；无历史表：面板难以管理已入库文件                                      |
...
| **备选缺陷** | 原始 query 直接 embedding：Sparse 路无词项；LLM 查询改写：延迟与成本高，留作扩展      |
...
| **备选缺陷** | 本地暴力余弦：数据量大不可扩展；已保留 VectorStore 抽象以便换库                                             |
...
| **备选缺陷** | 仅 Dense：产品名/库名类 query 易漏召；外接 ES：多一跳运维                             |
...
| **备选缺陷** | 线性加权：两路分数量纲不同难调参；单路：丢失互补能力                              |
...
| **备选缺陷** | 仅粗排：Top-3 展示时常「沾边但排后」；Cohere API：额外 Key 与费用                                                              |
...
| **备选缺陷** | 仅应用日志：难对照阶段命中数；OpenTelemetry：RAG 字段需大量定制，过重                                                                         |
...
| **备选缺陷** | FastAPI+React：迭代慢；Grafana：难做 chunk 级回放与入库操作                                                                     |


评估与 Trace 衔接见 [评估体系设计.md](./evaluation.md)。

---

## 5. 评估链路选型

体系设计与指标定义见 [评估体系设计.md](./evaluation.md)；此处仅列组件选型。
...
| **备选缺陷** | 纯人工 spot check：不可自动化；仅 Ragas：无法客观衡量「搜没搜到」                                    |
...
| **备选缺陷** | 自研 LLM 打分：指标口径难统一；DeepEval：生态与 Ragas 场景重叠，迁移收益小                                                            |
...
| **备选缺陷** | 数据库维护：版本与 pytest 同步麻烦；无固定集：改配置后无法客观对比                                                                     |
...
| **备选缺陷** | 自研 JSON-RPC：与生态 Client 不兼容；MCP over HTTP/SSE：需端口与部署，非当前主路径                   |
...
| **备选缺陷** | 同进程内嵌库调用：与 Agent 耦合、chromadb 懒加载易死锁                                       |
