---
id: analysis/display-optimization
title: 旅行模式 MCP 与 RAG 能力展示优化建议
tags: [analysis, travel, product, roadmap]
sources:
  - agents-master/ui/travel_evidence.py
  - agents-master/modes/travel/pipeline.py
  - rag-server/src/observability/
related: [host/travel-product, host/travel-impl, rag/observability]
updated: 2026-09-10
---
# 旅行模式 MCP 与 RAG 能力展示优化建议

> **TL;DR**：旅行模式「能力在、展示不在」——RAG 证据与 MCP 工具结果对用户不可见；本文给出证据层、工具卡片、数据管道 + 写作 Agent、交互式 RAG、可信度 UI 等 P0–P3 建议与落地顺序。

> 文档版本：v1.2  
> 更新日期：2026-06-16  
> 适用项目：`agents-master`（Streamlit + LangGraph ReAct + 旅行规划工作流）  
> 关联文档：[Token 消耗分析与优化](./token-optimization.md)、[多模式 Agent 架构选型](../10-host/modes.md)  
...
- [旅行规划-需求与架构(产品)](../10-host/travel-product.md)
- [旅行规划-职责分层与步骤依据(实现)](../10-host/travel-impl.md)
- [多模式 Agent 架构选型](../10-host/modes.md)
- [记忆系统现状与优化](wiki/10-host/memory.md)（L4 用户记忆 vs RAG 领域知识边界）
