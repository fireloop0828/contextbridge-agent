---
id: analysis/rag-issues
title: RAG 测试问题与优化记录
tags: [analysis, rag, retrieval, troubleshooting]
sources:
  - rag-server/src/core/query_engine/
  - rag-server/src/ingestion/
  - rag-server/src/observability/dashboard/
related: [rag/architecture, analysis/rag-evaluation-tuning, rag/observability]
updated: 2026-09-10
---
# RAG 测试问题与优化记录

> **TL;DR**：RAG 主链路踩坑台账，按「检索 / 入库 / Dashboard」分组逐条记录现象、根因与改动文件，可直接用于排障与面试复盘。

> 开发笔记 · 记录入库、检索、Dashboard、MCP 联调过程中的排障与优化，体现项目迭代轨迹。评估专项见 [RAG评估调优专题.md](./rag-evaluation-tuning.md)。
...
- **处理**：
  ...
  - Agent 侧先 `list_collections` 再 `query_knowledge_hub(..., collection="travel_plan")`（见 [外接集成设计.md](wiki/20-rag/mcp-server.md) §3.5）。
...
- **处理**：
  ...
  - **使用的技术 / 改动文件**：`hybrid_search.py`、`sparse_retriever.py`、`dense_retriever.py`、`bm25_indexer.py`；架构说明见 [系统架构与模块选型.md](wiki/20-rag/architecture.md) §2.6、§3.2。
...
- **结果 / 待验证**：主链路排障基本不必手抠 JSONL；评估类问题仍见 [RAG评估调优专题.md](./rag-evaluation-tuning.md)。
...
- **处理**：
  ...
  - **使用的技术 / 改动文件**：`agents-master/config.json`；`agents-master/config/mcp_config.py`；说明见 [外接集成设计.md](wiki/20-rag/mcp-server.md) §5。
...
- **处理**：
  ...
  - **新增库维护**：入库（`--collection <名>`）→ yaml 增加同名条目 → 重启 MCP → `list_collections` 验证；详见 [知识库扩展计划.md](../../rag-server/docs/知识库扩展计划.md) §8。
  - **使用的技术 / 改动文件**：
