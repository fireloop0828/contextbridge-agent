...
tags: [host, memory]
sources:
  - agents-master/memory_store.py
  - agents-master/memory_recall.py
  - agents-master/session_store.py
  - agents-master/memory_pipeline.py
related: [host/architecture, host/middleware, analysis/token-optimization]
updated: 2026-09-10
---

# Agent 对话记忆系统

> **TL;DR**：记忆分四层——L1 会话历史（LangGraph MemorySaver）、L2 会话归档、L3 结构化事实、L4 向量长期记忆；各层独立写入与读出，L4 用 Embedding 做跨会话画像召回。
...
**旅行模式额外（会话内提炼，进 L3 / 下轮 prompt，非独立层）：** `travel_intake`、`modes/travel/tool_memory`（最近 6 回合）、`data/outputs/*.md`（正文在磁盘，记忆层只存路径）。
...
| A2  | [旅行] `handler.prepare_before_agent`：合并 intake、算 phase，拼 `[TRAVEL_CONTEXT]`    | 会话内提炼    |
...
2. **旅行模式 Token 治理：** O2/O4 换 `thread_id` + `modes/travel/tool_memory` 外置摘要，是 **checkpoint 裁剪 + 外部结构化记忆** 的组合，比单纯 Message Trimmer 更可解释。
...
| 归档 pipeline 同步调 LLM  | **已实现：后台线程**（先写盘归档，LLM 摘要后台补）           | 归档按钮不阻塞 UI  |
...
| 旅行提炼  | `modes/travel/state_machine.py`、`modes/travel/tool_memory.py` | CONTEXT / thread 切换    |
