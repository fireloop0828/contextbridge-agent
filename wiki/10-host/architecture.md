...
title: Host 架构与 app.py 拆分
tags: [host, architecture, app]
sources:
  - agents-master/app.py
  - agents-master/ui/sidebar.py
  - agents-master/modes/travel/state_machine.py
related: [host/middleware, host/prompts, host/mcp-client]
updated: 2026-09-10
---

# app.py 架构说明与拆分建议

> **TL;DR**：`app.py` 是 Host 主入口，只保留「Agent 与 MCP 生命周期、旅行导出写盘、Streamlit 装配」三类职责；UI、旅行编排与 Prompt 已分别外迁到 `ui/`、`modes/travel/`、`prompts/`，文件从约 1675 行瘦身到约 668 行。
...
| 文档 | 说明 |
|------|------|
| [MCP设计与管理.md](./mcp-client.md) | MCP 流程、注册表、`initialize_session` / 重连策略 |
| [旅行规划-职责分层与步骤依据(实现).md](./travel-impl.md) | Agent / 编排 / Prompt 分工 |
| [旅行规划-需求与架构(产品).md](./travel-product.md) | 产品需求与架构 |
| [多模式Agent架构选型.md](./modes.md) | 三模式 + registry 设计 |
| [../test-analysis/MCP初始化性能分析与优化.md](../50-analysis/mcp-init-performance.md) | MCP 连接性能优化 |
