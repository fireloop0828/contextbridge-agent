---
id: host/travel-impl
title: 旅行规划 · 职责分层与步骤依据（实现）
tags: [host, travel, impl]
sources:
  - agents-master/modes/travel/handler.py
  - agents-master/modes/travel/pipeline.py
  - agents-master/modes/travel/facts.py
related: [host/travel-product, host/architecture, host/modes]
updated: 2026-09-10
---

# 旅行规划：职责分层与步骤依据

> **TL;DR**：旅行模式由三方分工：Python 编排层预取 RAG/高德事实并写入 `[TRAVEL_CONTEXT]`，Agent 在状态机约束下撰写正文与补调工具，Prompt 约束工具顺序与可信度规则。

> 说明「旅行规划 Agent 到底干什么」以及「每一步依据什么操作」。  
> 配套文档：[旅行规划-需求与架构(产品).md](./travel-product.md)
...
|------|----------|--------|----------------------|
| **编排层（Orchestrator）** | `modes/travel/`（`handler.py` + `state_machine.py` + `pipeline.py`） | 状态机、字段解析、预取、消息包装、交付校验 | **决定当前 phase**、拼接 `[TRAVEL_CONTEXT]`、合并 intake |
| **Agent 运行时** | LangGraph `create_react_agent` | LLM ↔ 工具多轮循环（ReAct） | **理解上下文、选择工具、生成回复** |
| **Prompt 层** | `prompts/general_system.md` + `travel-planner.md` + 每轮 `[TRAVEL_CONTEXT]` | 告诉模型「该怎么想、该调什么」 | **静态能力说明 + 本回合阶段指令** |
...
flowchart TB
    ...
    subgraph Orch["编排层 modes/travel/ + ui/chat.py"]
        D[travel_phase 状态机]
        E[travel_intake 九字段]
    ...
    subgraph Agent["LangGraph ReAct Agent"]
        G[travel-planner.md + general_system_travel.md]
        H[多轮 LLM ↔ 工具调用]
    end
...
用户输入
    ...
    │
    ▼
travel_handler.prepare_before_agent()              ← modes/travel/handler.py（由 ui/chat.py 调用）
    │
...
拼 agent_query = [TRAVEL_CONTEXT] JSON + 阶段指令 + 用户原文
    ...
    ▼
ReAct Agent
    System Prompt = general_system_travel.md + travel-planner.md（旅行模式）
    │
    ...
    │
    ▼
LLM 决定：提问 / 写攻略正文（Markdown）/（按需）调 maps_* / 调 RAG  
系统负责：自动导出写盘（不再要求 LLM 调 `write_markdown_document`）
    │
    ▼
travel_handler.process_after_agent()               ← modes/travel/handler.py
    │
...
6. （无）写盘由服务端自动导出（检测到完整攻略后写入 `data/outputs/`）
...
| 维度 | 通用模式 | 旅行规划 |
|------|----------|----------|
| Prompt | `prompts/general_system.md` | `general_system_travel.md` + `travel-planner.md` |
...
| 事件 | 行为 |
|------|------|
| 侧边栏选「旅行规划」 | `modes.travel.on_enter()`（经 `modes.registry`），重置 intake，切换 System Prompt |
...
> 编排层管 **状态与门控**（该问还是该生成）；Prompt 管 **能力与格式**；LLM 管 **具体工具调用与文案**；`ui/chat.py` + `app.py` 管 **UI 与运行时装配**。

相关代码入口：

- 状态机与上下文：`modes/travel/state_machine.py` — `advance_travel_turn`、`compose_travel_agent_query`、`check_travel_delivery`
- 回合编排：`modes/travel/handler.py` — `prepare_before_agent`、`prefetch_facts`、`process_after_agent`
- MCP 预取：`modes/travel/pipeline.py` — `prefetch_travel_facts`
- 聊天入口：`ui/chat.py` — 意图路由、旅行回合、导出挂载
- 自动写盘：`app.py` — `auto_export_travel_plan_from_chat`
- 静态 Prompt：`modes/travel/travel-planner.md`、`modes/travel/general_system_travel.md`
