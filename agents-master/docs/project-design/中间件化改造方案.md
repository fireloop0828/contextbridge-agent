...
| 文档 | 说明 |
|------|------|
| [app.py架构说明与拆分建议.md](./app.py架构说明与拆分建议.md) | app.py 职责边界与既有拆分 |
| [多模式Agent架构选型.md](./多模式Agent架构选型.md) | 模式包 + registry 设计（TravelMiddleware 的底座） |
| [旅行规划-职责分层与步骤依据(实现).md](./旅行规划-职责分层与步骤依据(实现).md) | Agent / 编排 / Prompt 分工 |
| [../test-analysis/记忆系统现状与优化.md](../test-analysis/记忆系统现状与优化.md) | 记忆注入/抽取现状（Memory 类中间件的依据） |
