...
| P2-3 | Prompt 调整为 **「基于 [TRAVEL_FACTS] 写作」**，删除长工具 checklist                     | `travel_mode.py` `GENERATING_TOOL_CHECKLIST`；`travel-planner.md` | System/context 更短；模型专注排版与可行性叙述      | §4.3         | Token 较 P0 再降（用 `estimate_travel_token_savings.py` 记一笔） |
